		...
		<script>
			var vue = new Vue({
				...
				data: {
					...
					dataList: [],
                    peiwanrenyuanOrderPaymentTypes: 1,
                    zhekou:1,
					// 当前用户信息
					user: {}
				},
				...
				methods: {
					...
					,payClick() {
						let data={
                            peiwanrenyuanId: localStorage.getItem('userid'),
                            yonghuId: localStorage.getItem('userid'),
                            peiwanrenyuanOrderPaymentTypes: vue.peiwanrenyuanOrderPaymentTypes,
						}
                        // 获取商品详情信息
                        layui.http.request(`peiwanrenyuanOrder/order`, 'POST', data, (res) => {
                            // 订单编号
                            var orderId = genTradeNo();
							if(res.code == 0){
                                alert("下单成功，点击确定后跳转 我的订单页面");
                                window.location.href='../peiwanrenyuanOrder/list.jsp';
							}
                       		 window.location.href='../peiwanrenyuanOrder/list.jsp';
					    });
					}
