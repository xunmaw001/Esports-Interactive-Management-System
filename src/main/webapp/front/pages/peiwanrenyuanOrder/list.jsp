...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的陪玩人员订单 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="modal-dialog" style="z-index: 3000">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <textarea id="peiwanrenyuanCommentbackContent" style="width: 500px;height: 300px"></textarea>
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" @click="submitPeiwanrenyuanCommentback()" class="btn btn-theme">评价</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                    <li class="layui-this" @click="pageList(0)">全部陪玩人员订单</li>
                    <li v-for="item in peiwanrenyuanOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                </ul>
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            ...
                            <td style="width: 60px;">{{item.peiwanrenyuanName}}</td>
                            <td style="width: 150px;">{{item.peiwanrenyuanOrderUuidNumber}}</td>
                            <td style="width: 80px;">{{item.youxiValue}}</td>
                            <td style="width: 150px;">{{item.kaishiTime}}</td>
                            <td style="width: 150px;">{{item.buyNumber}}</td>
                            <td style="width: 150px;">{{item.peiwanrenyuanOrderTruePrice}}</td>
                            <td style="width: 80px;">{{item.peiwanrenyuanOrderValue}}</td>
                            <td style="width: 80px;">{{item.peiwanrenyuanOrderPaymentValue}}</td>
                            <td style="width: 150px;">{{item.insertTime}}</td>
                            <td style="width: 100px;">
                                <button v-if="item.peiwanrenyuanOrderTypes==1" @click="refund(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                ...
                                <button v-if="item.peiwanrenyuanOrderTypes==3" @click="receiving(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                    <i class="layui-icon">&#xe65e;</i> 用户确认
                                </button>




                                <button v-if="item.peiwanrenyuanOrderTypes==4" @click="commentback(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius btn-theme">
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            //列表查询
            ,pageList(peiwanrenyuanOrderTypes) {
                this.peiwanrenyuanOrderTypes = peiwanrenyuanOrderTypes;
                // 获取列表数据
                layui.http.request('peiwanrenyuanOrder/page', 'get', {
                    page: 1,
                    limit: this.limit,
                    peiwanrenyuanOrderTypes: (vue.peiwanrenyuanOrderTypes == 0)?null:vue.peiwanrenyuanOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('peiwanrenyuanOrder/page', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    peiwanrenyuanOrderTypes: (this.peiwanrenyuanOrderTypes == 0)?null:this.peiwanrenyuanOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`peiwanrenyuanOrder/pay?id=`+id, 'GET',{}, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
                            time: 2000,
                            icon: 6
            ...
            ,refund(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`peiwanrenyuanOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
            ...
            ,receiving(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`peiwanrenyuanOrder/receiving?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('成功用户确认', {
                            time: 2000,
                            icon: 6
            ...
            ,submitPeiwanrenyuanCommentback(){
                var peiwanrenyuanCommentbackContent = $("#peiwanrenyuanCommentbackContent").val();
                var id = $("#id").val();

                layui.http.request("peiwanrenyuanOrder/commentback?id="+id+"&commentbackText="+peiwanrenyuanCommentbackContent, 'get', {}, function (res) {
                    if(res.code==0){
                        layer.msg('评价成功', {
                            time: 2000,
                            icon: 6
    ...
    // 当前表的字典表
        ...
        function peiwanrenyuanOrderTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=peiwanrenyuan_order_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function peiwanrenyuanOrderPaymentTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=peiwanrenyuan_order_payment_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        // 获取列表数据
        http.request('peiwanrenyuanOrder/page', 'get',{
            page: 1,
            limit: limit,
            peiwanrenyuanOrderTypes: (vue.peiwanrenyuanOrderTypes == 0)?null:vue.peiwanrenyuanOrderTypes
        }, function(res) {
            ...
            laypage.render({
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('peiwanrenyuanOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            peiwanrenyuanOrderType: (vue.peiwanrenyuanOrderTypes == 0)?null:vue.peiwanrenyuanOrderTypes
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
