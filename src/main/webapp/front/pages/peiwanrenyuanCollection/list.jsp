                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="peiwanrenyuanUuidMumber" id="peiwanrenyuanUuidMumber" placeholder="陪玩人员编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="peiwanrenyuanName" id="peiwanrenyuanName" placeholder="陪玩人员姓名" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                        ...
                        <div class="team-info">
                            <div class="team-row">
                                <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 col-sm-6 col-xs-6 team-grids">
                                    <i @click="deletePeiwanrenyuanCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                    <div class="team-agile-img" @click="jumpCheck('../peiwanrenyuan/detail.jsp?id='+item.peiwanrenyuanId , item.peiwanrenyuanDelete == null?'':item.peiwanrenyuanDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                        <img :src="item.peiwanrenyuanPhoto?item.peiwanrenyuanPhoto.split(',')[0]:''" style="height: 350px;width:100%;" />
                                        <div class="view-caption">
                                            <div class="w3ls-info">
                                                <h4>{{item.peiwanrenyuanName}}</h4>
                                            </div>
                                            <b v-if="item.peiwanrenyuanNewMoney" style="float: right;display: inline-block;color: #FFFFFF;font-size: 17px;">
                                                {{item.peiwanrenyuanNewMoney}} RMB
                                            </b>
                                            <b v-else style="float: right;display: inline-block;color: #FFFFFF;font-size: 17px;">
                                                {{item.peiwanrenyuanValue}}
                                            </b>
                                        </div>
                                    </div>
                                </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            peiwanrenyuanCollectionTypesList : [],
        // 级联表字典表数据
            peiwanrenyuanCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deletePeiwanrenyuanCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('peiwanrenyuanCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function peiwanrenyuanCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=peiwanrenyuan_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.peiwanrenyuanCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function pageList() {
            ...
            if (jquery('#peiwanrenyuanUuidMumber').val() != null && jquery('#peiwanrenyuanUuidMumber').val() != "" && jquery('#peiwanrenyuanUuidMumber').val() != "null" ) {
                param['peiwanrenyuanUuidMumber'] = jquery('#peiwanrenyuanUuidMumber').val();
            }
                     
            if (jquery('#peiwanrenyuanName').val() != null && jquery('#peiwanrenyuanName').val() != "" && jquery('#peiwanrenyuanName').val() != "null" ) {
                param['peiwanrenyuanName'] = jquery('#peiwanrenyuanName').val();
            }
                                                                                                                                            
            // 当前表
                                                
            //收藏只能查类型为1的
            param['peiwanrenyuanCollectionTypes'] = 1;

            // 获取列表数据
            http.request('peiwanrenyuanCollection/page', 'get', param, function(res) {
